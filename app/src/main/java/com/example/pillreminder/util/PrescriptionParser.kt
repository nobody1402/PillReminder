...
object PrescriptionParser {
    ...
    private fun parseLine(originalLine: String): ParsedPrescriptionItem? {
        ...
        // استخراج تعداد از روی کلمه «تعداد»
        val qtyMatch = Regex("تعداد\\s*([\\d۰-۹]+)").find(line)
        ...
        // حذف اعداد/واحدهای دوز باقی‌مانده (مثل "۵۰۰ میلی گرم"، "۱/۳ – ۲/۳ نیم لیتری"، "۶۴۳")
        val cleanedName = line
            .replace(Regex("[\\d۰-۹/.\\-–]+"), " ")
            .replace(Regex("میلی\\s*گرم|میلی\\s*لیتر|گرم|لیتری|نیم|cc|mg|ml"), " ")
            .replace(Regex("\\s+"), " ")
            .trim()
