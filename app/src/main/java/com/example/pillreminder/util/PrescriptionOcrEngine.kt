    ...
    suspend fun recognize(context: Context, bitmap: Bitmap): OcrResult = withContext(Dispatchers.Default) {
        ...
        return@withContext try {
            ...
            if (!ok) {
                ...
                // تشخیص خودکار چیدمان صفحه (نه یک بلوک یکپارچه) — برای فرمت‌های جدولی حیاتیه:
                // اگه کل صفحه رو یک بلوک فرض کنیم، متنِ ستون‌های کنار هم با هم قاطی می‌شه
                // (کاراکترهای فارسی و انگلیسی از دو ستون مختلف به‌هم می‌چسبن). با تشخیص
                // خودکار چیدمان، Tesseract خودش ستون‌ها/سطرهای جدول رو تشخیص می‌ده.
